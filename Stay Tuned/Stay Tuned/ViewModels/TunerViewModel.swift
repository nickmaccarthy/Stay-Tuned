...
final class TunerViewModel: ObservableObject {
    ...
    @AppStorage("lastSelectedTuningId") private var lastSelectedTuningId: String = "guitar_standard"
    @AppStorage("referencePitch") var referencePitch: Int = 440
    @AppStorage("tunerMode") private var storedTunerMode: String = TunerMode.instrument.rawValue
    @AppStorage("toneType") private var storedToneType: String = ToneType.string.rawValue
    ...
    @Published var tunerMode: TunerMode = .instrument {
        didSet {
    ...
    @Published var detectedNote: ChromaticNote?
    @Published var detectedFrequency: Double = 0
    @Published var centsDeviation: Int = 0
    @Published var isInTune: Bool = false
    @Published var isListening: Bool = false
    @Published var hasPermission: Bool = false
    @Published var isDetectingPitch: Bool = false
    @Published var autoDetectString: Bool = true  // Auto-detect which string user is tuning
    
    @Published var tunedStrings: Set<Int> = []  // For auto-advance feature
    @Published var confirmedStrings: Set<Int> = []  // Strings that have been confirmed in-tune (stays green)
    @Published var showTuneConfirmation: Bool = false
    @Published var allStringsTuned: Bool = false
    @Published var showAllTunedFlash: Bool = false  // Quick flash when all strings tuned
    @Published var sustainedInTune: Bool = false  // True when in tune for 2+ seconds
    @Published var currentDecibels: Float?  // Current dB level when detecting
    
    // Audio samples for spectrum analyzer
    @Published var audioSamples: [Float] = []
    @Published var audioSampleRate: Double = 48000
    ...
    @Published var isPlayingTone: Bool = false
    @Published var playingToneStringId: Int? = nil  // Which string's tone is currently playing
    ...
    private var inTuneStartTime: Date? = nil
    ...
    private func processAudio(samples: [Float], sampleRate: Double) {
        ...
        let isSustainedInTune: Bool
        if let startTime = inTuneStartTime {
            isSustainedInTune = Date().timeIntervalSince(startTime) >= sustainedInTuneDuration
        } else {
            isSustainedInTune = false
        }
